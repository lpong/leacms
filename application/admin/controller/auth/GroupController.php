...
class GroupController extends CommonController
{
    ...
    public function sort()
    {
        ...
        if ($id > 0 && Db::name('auth_group')->where('id', $id)->setField('sort', $sort) !== false) {
            $this->success('设置成功');
        }
    ...
    public function assigned()
    {
        if ($this->request->isPost()) {
            ...
            $have_rule = Db::name('auth_rule')->where('id', 'in', $info['rules'])->order('pid asc,id asc')->select();
            $no_rule   = Db::name('auth_rule')->where('id', 'not in', $info['rules'])->order('pid asc,id asc')->select();
            if ($ajax) {
